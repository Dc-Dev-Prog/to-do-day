...
function New-ReporteSaldo {
    ...
    # Crear carpeta específica de la red en la nueva estructura
    $carpetaRed = Join-Path (Split-Path $PSScriptRoot) "reports\balance\$Red"
    if (-not (Test-Path $carpetaRed)) {
        New-Item -Path $carpetaRed -ItemType Directory -Force | Out-Null
    }
    
    $archivo = Join-Path $carpetaRed "balance-$Red-report-$timestamp.md"
