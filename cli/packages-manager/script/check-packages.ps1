...
function New-ReporteRed {
    ...
    $carpetaRed = Join-Path (Split-Path $PSScriptRoot) "reports\check-packages\$Red"
...
"@
        foreach ($paquete in $paquetes.GetEnumerator()) {
            $contenido += "|``$($paquete.Key)`` | ``$($paquete.Value.UpgradeCap)`` | $($paquete.Value.Version) | [🔗](https://suiexplorer.com/object/$($paquete.Key)?network=$Red) |`r`n"
        }
    }
    else {
        $contenido += @"
...
Write-Host "`n✅ Verificación completa! Reportes guardados en .\reports\check-packages\" -ForegroundColor Green
